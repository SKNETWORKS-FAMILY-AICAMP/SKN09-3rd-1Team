...
# 1️⃣ **프로젝트 개요**
## ✅ **프로젝트 명: LawQuick**
## ✅ **프로젝트 소개: LawQuick_이혼 특화 AI 법률 상담 챗봇**
...
**국가법령정보센터(https://www.law.go.kr/) OPENAPI** 이용해 **“이혼” 관련 키워드**로 판례 데이터 **크롤링** 진행 
...
| 사건명 | 사건의 이름 | `string` |
| 사건번호 | 법원에서 부여한 사건 번호 | `string` |
| 선고일자 | 판결 선고 일자 (`YYYY-MM-DD`) | `string (date)` |
| 법원명 | 사건을 판결한 법원 명칭 | `string` |
| 사건종류명 | 사건의 종류 (예: 형사, 민사) | `string` |
| 사건종류코드 | 사건 종류에 대한 코드 | `string` |
| 판결유형 | 판결 방식 (예: 판결, 결정) | `string` |
| 선고 | 선고 여부 | `string` |
| 판시사항 | 판결에서 문제된 주요 법적 쟁점 | `string` |
| 판결요지 | 판결의 핵심 요지 | `string`  |
| 참조조문 | 참조한 법령 조문 | `string` |
| 참조판례 | 판결에서 참조한 기존 판례 | `string` |
| 판례내용 | 상세한 판결 정보 포함 (중첩 구조) | `object` |
...
| 필드명 | 설명 | 데이터 타입 |
| --- | --- | --- |
| 법령명 | 법률명, 여러 조항을 포함한 리스트 | `array` |

### 🔹 법령 항목

| 필드명 | 설명 | 데이터 타입 |
| --- | --- | --- |
| 법률명 | 해당 항목이 속한 법률의 명칭 | `string` |
| 조항번호 | 법 조문의 번호 | `string` |
| 조항제목 | 조문의 제목 또는 주제 | `string` |
| 조항내용 | 조항 내 세부 항 내용이 담긴 객체 | `object` |

### 🔸 조항내용

| 필드명 | 설명 | 데이터 타입 |
| --- | --- | --- |
| 항 번호 | 각 조항의 내용 | `string` |

- 총 70개의 법률, 전체 5819개 조항의 법령 데이터 확보
