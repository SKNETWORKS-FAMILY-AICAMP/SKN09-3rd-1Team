...
- **사용 목적**: 가족법 전문 챗봇 학습을 위한 질의응답 데이터 생성
- **데이터 구성 방식**:
    - 사용자 정보 기반 시나리오 300건 생성
    - 각 시나리오당 3개의 법률 질문 생성 → 총 900개 질문
    - 질문별로 RAG 검색된 문서 내용을 기반으로 구조화된 답변 생성
- **최종 확보량**: **900개 Instruction-Input-Output 쌍**
...
| input | 질문에 대한 배경 정보 (사용자 정보 + RAG 문서) |
| output | 질문과 input에 기반한 모델의 답변  |
----
# 7️⃣ **DB 연동 구현 코드**
